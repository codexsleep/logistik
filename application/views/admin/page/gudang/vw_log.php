<div class="content-page">
                            ...
                            <div class="row justify-content-between">
                                <div class="col-auto">
                                    <form class="d-flex flex-wrap align-items-center" action="?fil" method="GET">
                                        ...
                                        <div class="me-sm-3">
                                            <select class="form-select my-1 my-lg-0" id="barang-select"
                                                data-toggle="select2" name="filbarang" style="width:200px;">
                                ...
                                <div class="col-auto">
                                    <div class="text-lg-end my-1 my-lg-0">
                                        ...
                                        <a href="<?= base_url() ?>admin/gudang/log"
                                ...
                                <div class="col-sm-4">
                                    <button type="button" class="btn btn-danger waves-effect waves-light"
                                        data-bs-toggle="modal" data-bs-target="#tambah-log" id="tambah-logbtn"><i
                                            class="mdi mdi-plus-circle me-1"></i> Tambah Log Barang</button>
                                </div>
                                <div class="col-sm-8">
                                    <div class="text-sm-end mt-2 mt-sm-0">
                                        <button type="button" class="btn btn-success mb-2 me-1" id="filter-btn"><i
                                                class="mdi mdi-cog"></i></button>
                                        <button type="button" class="btn btn-light mb-2 me-1" onclick="showSweetAlert()">Import</button>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header bg-light">
                    <h4 class="modal-title" id="myCenterModalLabel">Tambah Log Barang</h4>
                ...
                <div class="modal-body p-4">
                    ...
                    <form>
                        ...
                        <div class="mb-3" id="outletInput">
                            <label for="outlet" class="form-label">Outlet Tujuan</label>
                            <select class="form-control" id="outlet-input" data-toggle="select2" data-width="100%"
                                name="outlet">
  ...
  function showSweetAlert() {
    Swal.fire({
      title: 'Pilih Jenis Import',
      icon: 'question',
      showCancelButton: true,
      confirmButtonText: 'Barang Masuk',
      cancelButtonText: 'Barang Keluar',
      showCloseButton: true,
      closeButtonText: 'Batal'
    }).then((result) => {
        if (result.isConfirmed) {
        window.location.href = '<?= base_url();?>admin/gudang/import_masuk'; // Ganti dengan URL halaman masuk
      } else if (result.dismiss === Swal.DismissReason.cancel) {
        window.location.href = '<?= base_url();?>admin/gudang/import_keluar'; // Ganti dengan URL halaman keluar
      }
    });
  }
        ...
        exportBtn.addEventListener('click', function () {
            ...
            if (!filbarang || filbarang === '' || filbarang.toLowerCase() === 'all') {
                ...
                const exportUrl = url.origin + "/admin/gudang/export_logbarang?filstart=" + filstart + "&filend=" + filend + "&filbarang=" + filbarang + "&filstatus=" + filstatus;
        ...
        var outletInput = document.getElementById("outletInput");
        ...
        outletInput.style.display = "none";
        ...
        jenisInput.onchange = function () {
            if (jenisInput.value === "1") { // Log Masuk
                ...
                outletInput.style.display = "block";
                keteranganInput.style.display = "none";
                jumlahInput.style.display = "block";
                tanggalInput.style.display = "block";
                $(document).ready(function () {
                    ...
                    $.ajax({
                        url: "<?= base_url(); ?>admin/gudang/getOutlet",
                        method: 'POST',
                        dataType: 'json',
                        success: function (response) {
                            ...
                            $('#outlet-input').append(
                                $('<option></option>').val('').html('Pilih Outlet')
                            );

                            $.each(response, function (index, data) {
                                $('#outlet-input').append(
                                    $('<option></option>').val(data.id).html(data.nama_outlet)
                                );
                            });
                        },
        ...
        $(document).ready(function () {
            $('#tambah-logbtn').click(function () {
                $.ajax({
                    url: "<?= base_url(); ?>admin/gudang/getBarang",
                    method: 'POST',
                    dataType: 'json',
                    success: function (response) {
                        ...
                        $.each(response, function (index, data) {
                            $('#barang-input').append(
                                $('<option></option>').val(data.id).html(data.nama_barang)
                            );
                        });
                    },
                    error: function (xhr, ajaxOptions, thrownError) {
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        $.each(response, function (index, data) {
                            ...
                            $('#barang-select').append(
                                $('<option></option>').val(data.id).html(data.nama_barang).prop('selected', selected)
                            );
                        });
                    },
                    error: function (xhr, ajaxOptions, thrownError) {
